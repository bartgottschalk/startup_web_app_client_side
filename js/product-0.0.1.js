...
load_product = function( data, textStatus, xhr ) {
	...
	var title_url = data['product_data']['title_url'];
	var identifier = data['product_data']['identifier'];
	...
	for (product_image_id in product_images_raw_data) {
		var image_dict = {};
	...
	for (product_video_id in product_videos_raw_data) {
		var video_dict = {};
	...
	for (sku_id in skus_raw_data) {
		...
		for (sku_image_id in sku_images_raw_data) {
			var image_dict = {};
	...
	for (image_id in images_dict['product']) {
	...
	for (image_id in images_dict['sku']) {
		if (image_count >= 2) {
	...
	for (image_id in images_dict['video']) {
		if (image_count >= 2) {
...
sku_radio_item_changed = function(selected_sku_id) {
	...
	for (sku_id in skus_raw_data) {		
		if (selected_sku_id == sku_id) {
			...
			for (image_counter in images_dict['sku']) {
